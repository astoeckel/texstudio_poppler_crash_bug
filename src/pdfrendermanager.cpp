...
SetImageForwarder::SetImageForwarder(QObject *parent, QObject *obj,
                                     const char *rec, QPixmap img, int pageNr)
...
{
	QMetaObject::invokeMethod(obj, rec, Q_ARG(QPixmap, img),
	                          Q_ARG(int, pageNr));
	deleteLater();
}
...
CachePixmap::CachePixmap() : QPixmap(), resolution(0), x(0), y(0) {}
 ...
 * Negative number means limit number to abs() if that number is smaler than
 * cores, positive number sets it to the given value.
 *
 */
    ...
    : QObject(parent), cachedNumPages(0), loadStrategy(HybridLoad)
{
	...
	for (int i = 0; i < queueAdministration->num_renderQueues; i++) {
		...
		connect(renderQueue, SIGNAL(sendImage(QImage, int, int)), this,
		        SLOT(addToCache(QImage, int, int)));
...
void PDFRenderManager::stopRendering()
{
	...
	for (int i = 0; i < queueAdministration->num_renderQueues; i++) {
		if (queueAdministration->renderQueues[i] &&
		    !queueAdministration->renderQueues[i]->isRunning())
...
QSharedPointer<Poppler::Document> PDFRenderManager::loadDocument(
    const QString &fileName, Error &error, const QString &userPasswordStr,
    bool foreceLoad)
{
	renderedPages.clear();
	QFile f(fileName);
	...
	try {
		if (loadStrategy == DirectLoad) {
			docPtr =
			    Poppler::Document::load(fileName, ownerPassword, userPassword);
		}
		else {
			// load data into buffer and check for completeness
			queueAdministration->documentData = f.readAll();
			if (!queueAdministration->documentData
			         .mid(qMax(0,
			                   queueAdministration->documentData.size() - 1024))
			         .trimmed()
			         .endsWith("%%EOF") &&
			    !foreceLoad) {
			...
			// create document
			if (loadStrategy == BufferedLoad ||
			    (loadStrategy == HybridLoad &&
			     queueAdministration->documentData.size() < 50000000)) {
				if (queueAdministration->documentData.size() < 1024)
					queueAdministration->documentData.append(QByteArray(
					    1024 - queueAdministration->documentData.size(),
					    (char)0));
				docPtr = Poppler::Document::loadFromData(
				    queueAdministration->documentData, ownerPassword,
				    userPassword);
			}
			else {
				docPtr = Poppler::Document::load(fileName, ownerPassword,
				                                 userPassword);
			}
		}
	}
	...
	QColor paperColor = QColorConstants::White;
	...
	queueAdministration->documentData
	    .clear();  // remove file, as poppler made a copy of its own
...
QPixmap PDFRenderManager::renderToImage(int pageNr, QObject *obj,
                                        const char *rec, double xres,
                                        double yres, int x, int y, int w, int h,
                                        ...
                                        Poppler::Page::Rotation rotate)
{
	if (document.isNull())
		return QPixmap();
	if (pageNr < 0 || pageNr >= cachedNumPages)
		return QPixmap();
	RecInfo info;
	info.obj = obj;
	info.slot = rec;
	...
	if (renderedPages.contains(pageNr + kMaxPageZoom) &&
	    xres > kMaxDpiForFullPage) {  // try cache first
		CachePixmap *cachedPix = renderedPages[pageNr + kMaxPageZoom];
		if (cachedPix->getCoord() == QPoint(x, y) &&
		    cachedPix->getRes() < 1.01 * xres &&
		    cachedPix->getRes() > 0.99 * xres && cachedPix->width() == w &&
		    cachedPix->height() == h) {
			img = *cachedPix;
			partialImage = true;
			enqueueCmd = false;
	...
	// delayTimeout >= 0 means it's been called called by delayedUpdate and
	// delayedUpdate wants to force an update after delayTimeout Note that when
	// delayTimeout >= 0 is used, and there's no cache, the slot can be called
	// with null img.
	if (delayTimeout >= 0) {
		if (!img.isNull())
			QMetaObject::invokeMethod(info.obj, info.slot, Q_ARG(QPixmap, img),
			                          Q_ARG(int, pageNr));
		else
			(new SetImageForwarder(this, obj, rec, img, pageNr))
			    ->forward(delayTimeout);
	}

	// if(img.isNull()) // not cached, thumbnail present ? (fix crash?)
	//	img=QPixmap::fromImage(page->thumbnail());
	if (!img.isNull() &&
	    !partialImage) {  // if a image was found, scale it apropriately
		...
		if (scale > 1.01 || scale < 0.99) {
			img = img.scaled(QSize(sx, sy), Qt::KeepAspectRatio,
			...
			if (cache) {
				...
				int sizeInMB =
				    qCeil(image->width() * image->height() * image->depth() /
				          8388608.0);  // 8(bits depth -> bytes) * 1024**2
				                       // (bytes -> MB)
	...
	if (enqueueCmd) {
		if (scale > 1.01 || scale < 0.99) {  // always rerender, only not if it
			...
			while (i.hasNext()) {
				...
				if (mCurrentTicket != i.key()) {
					...
					if (elem.pageNr == info.pageNr && elem.obj == info.obj &&
					    elem.slot == info.slot) {
						qreal rel = elem.xres / info.xres;
						if (rel > 1.01 || rel < 0.99 || elem.x != info.x ||
						    elem.y != info.y || elem.w != info.w ||
						    elem.h != info.h) {
							i.remove();
						}
						else {
...
qreal PDFRenderManager::getResLimit() { return kMaxDpiForFullPage; }
...
void PDFRenderManager::fillCache(int pg)
{
	...
	while (i >= min || j < max) {
		j++;
		if (i >= min && i < max &&
		    !renderedPage.contains(i))  // don't rerender page
			renderToImage(i, nullptr, "");
		if (j >= min && j < max &&
		    !renderedPage.contains(j))  // don't rerender page
...
bool PDFRenderManager::checkDuplicate(int &, RecInfo &info)
{
	...
	for (; i != lstOfReceivers.constEnd(); ++i) {
		if (i.value().pageNr != info.pageNr)
			continue;
		if (i.value().x < 0 && i.value().y < 0 && i.value().w < 0 &&
		    i.value().h < 0 && info.x < 0 && info.y < 0 && info.w < 0 &&
		    info.h < 0) {
			if (info.priority) {
				// qDebug()<<"duplicate";
				return true;
